...
func (ds *App) Setup(ctx context.Context, dirs ...string) (err error) {
	...
	// Get user's addr book from local keybase
	...
	addr := resolveUnixOrTCPAddr(nodeCfg.TMConfig.RPC.ListenAddress)

	// Setup lazy proxy
	if ds.cfg.lazyLoader {
		proxyLogger := ds.logger.WithGroup(ProxyLogName)
		ds.proxy, err = proxy.NewPathInterceptor(proxyLogger, addr)
		if err != nil {
		...
		nodeCfg.TMConfig.RPC.ListenAddress = fmt.Sprintf("%s://%s", addr.Network(), addr.String())
	}
